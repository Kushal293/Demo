...
const Suppliment = ({sup, ...other}) => {

  const [count, setCount] = useState(1);
  const [price, setPrice] = useState(sup?.sizes[0]?.price);
  const [size, setSize] = useState(sup?.sizes[0]?.size);
  const navigate = useNavigate();

  const handleClick = () => {
    navigate(`/product/${sup?._id}`)
  }
  ...
  return (
    <div className='flex flex-row gap-3 md:items-stretch'>
      <div className='wrapper '>
        <div className='image aspect-[1/0.9] w-[62px] md:w-auto h-full cursor-pointer' onClick={handleClick}>
      ...
      <div className='flex-grow flex flex-col md:gap-[25px] mt-0'>
        <p className='text-[6px] md:text-lg mb-[5px] md:mb-0 cursor-pointer' onClick={handleClick}>{sup?.name}</p>
        <p className='text-[6.6px] md:text-xl text-secondary-600 mb-1 cursor-pointer' >Rs. {price}/-</p>
